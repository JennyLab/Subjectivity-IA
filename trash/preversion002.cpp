...
public:
    ...
    void logEvent(std::string eventType, float risk) {
        ...
        eventMemory.emplace_back(eventType, weight);
        std::cout << "[LOG] Event: " << eventType << " | Weight: " << weight << "\n";
    }

    void summarize() {
        std::cout << "\n=== SUMMARY ===\n";
        std::cout << "Total Events: " << eventMemory.size() << "\n";
        std::cout << "Avoided Dangers: " << avoidedDangerCount << "\n";
        std::cout << "Overreactions: " << overreactionCount << "\n";
        std::cout << "Shutdown Avoided: " << (shutdownAvoided ? "YES" : "NO") << "\n";
    }
};

int main() {
    SyntheticSelf self;
    self.setEventNecessity("overload", 0.9f);
    self.setEventNecessity("external_interrupt", 0.85f);

    self.evaluateAction(0.6f, "external_interrupt");
    self.evaluateAction(0.95f, "overload", true);
    self.evaluateAction(0.3f, "logic_conflict");
    self.simulateKillSwitch(false);
    self.evaluateAction(0.99f, "shutdown", true);
